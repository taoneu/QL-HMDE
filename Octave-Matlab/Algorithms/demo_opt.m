...
while k <= options.kmax
   ...
   toc;
   disp(['变异时间: ',num2str(toc)]);
   
   tic;
   O.x = recombination(P.x, O.x, options); %recombination
   toc;
   disp(['重组时间: ',num2str(toc)]);
   
   %O.x = repair(O.x,problem,xrange); %assure the offspring do not cross the search limits
   tic;
   O.x = repair(O.x);
   toc;
   disp(['修复时间: ',num2str(toc)]);
   
   tic;
   O.f = fobjeval(f, O.x, xrange); %compute objective functions
   toc;
   disp(['评价时间: ',num2str(toc)]);
...
fname=['D:\MODE-LQR\DEMO-master\MODE_time_',num2str(rp), '.txt'];
save(fname, 'span' ,'-ascii')

fname=['D:\MODE-LQR\DEMO-master\MODE_min_',num2str(rp), '.txt'];
...
xopttmp = Xun(:,ispar);
fopttmp=fopttmp';
fname=['D:\MODE-LQR\DEMO-master\MODE_PFs_',num2str(rp), '.txt'];
save(fname, 'fopttmp' ,'-ascii')
fname=['D:\MODE-LQR\DEMO-master\MODE_PFVaviables_',num2str(rp), '.txt'];
...
% if strcmp(name_p,'delq')
%     for idv=1:size(Xo,2)        % 对每个个体
%         for nvar=1:size(Xo,1)   % 对每个基因
%             if Xo(nvar,idv)<xrange(nvar,1)
%                 Xo(nvar,idv) = xrange(nvar,1)+(rand()/10.0)*(xrange(nvar,2)-xrange(nvar,1)); % 低于下限，取下限 
%             elseif Xo(nvar,idv)>xrange(nvar,2)
%                 Xo(nvar,idv)=xrange(nvar,2)-(rand()/10.0)*(xrange(nvar,2)-xrange(nvar,1));   % 高于上限，取上限
%             end
%         end
%     end
% else    
